...
func TestType_UnmarshalJSON(t *testing.T) {
	...
	tests := []struct{
		Datum []byte
		ExpectedValue cel_kind.Type
		ExpectedErr error
	}{
		{
			Datum: []byte(nil),
			ExpectedValue: cel_kind.Nothing(),
		...
		{
			Datum:                    []byte(`"apple"`),
			ExpectedValue: cel_kind.Something("apple"),
			ExpectedErr: nil,
		},
		{
			Datum:                    []byte(`"banana"`),
			ExpectedValue: cel_kind.Something("banana"),
			ExpectedErr: nil,
		},
		{
			Datum:                    []byte(`"cherry"`),
			ExpectedValue: cel_kind.Something("cherry"),
			ExpectedErr: nil,
		},
